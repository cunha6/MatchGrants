...
def _parse_detail(html: str, url: str) -> dict:
        ...
        for table in main_element.find_all("table"):
            ...
            for row in rows[1:]:
                ...
                if any(cells) and not any("global" in c.lower() for c in cells):
                    ...
                    fin_rows.append(row_dict)
    data["tabela_financeira"] = fin_rows

    dotacao_global = ""
    for row in fin_rows:
        if any("global" in str(v).lower() for v in row.values()):
            dotacao_global = next(
                (v for k, v in row.items() if "disponível" in k.lower()),
                ""
            )
            break
    data["dotacao_global"] = dotacao_global
    ...
    data["ultimo_aviso"] = (
        {"nome": docs[0]["nome"], "url": docs[0]["url"]} if docs else None
    )
