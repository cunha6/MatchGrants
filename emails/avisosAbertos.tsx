...
const avisosExemplo: Aviso[] = [
  {
    titulo: 'Inovação Produtiva — Portugal 2030',
    dotacao: '150 M€',
    taxaFinanciamento: '45%',
    url: 'https://www.aliados.consulting',
  },
  {
    titulo: 'Descarbonização da Indústria',
    dotacao: '60 M€',
    taxaFinanciamento: '60%',
    url: 'https://www.aliados.consulting',
  },
  {
    titulo: 'Qualificação das PME',
    dotacao: '25 M€',
    taxaFinanciamento: '50%',
    url: 'https://www.aliados.consulting',
  },
];

export default function AvisosAbertos({
  avisos = avisosExemplo,
}: AvisosAbertosProps) {
  const plural = avisos.length > 1;

  return (
    <Html>
      <Head />
      <Preview>
        {plural ? 'Novos avisos abertos' : 'Novo aviso aberto'} — candidaturas a decorrer
      </Preview>
      <Body style={main}>
        <Container style={container}>
          ...
          <Section style={hero}>
            ...
            <Heading style={heroHeading}>
              {plural ? 'Novos avisos abertos' : 'Novo aviso aberto'}
            </Heading>
            <Text style={heroText}>
              {plural
                ? 'Abriram novos avisos que podem enquadrar-se nos seus projetos. Consulte abaixo a dotação e a taxa de financiamento de cada um.'
                : 'Abriu um novo aviso que pode enquadrar-se nos seus projetos. Consulte abaixo a dotação e a taxa de financiamento.'}
            </Text>
            <Button style={pillButton} href="http://localhost:5173/newsletter/">
              {plural ? 'Ver detalhes dos avisos' : 'Ver detalhes do aviso'}
            </Button>
          </Section>
          ...
          <Section style={body}>
            <Row>
              <Column style={{ ...headerCell, width: '60%' }}>
                <Text style={headerText}>Aviso</Text>
              </Column>
              <Column style={{ ...headerCell, width: '25%' }}>
                <Text style={headerText}>Dotação</Text>
              </Column>
              <Column style={{ ...headerCell, width: '15%' }}>
                <Text style={headerText}>Taxa financ.</Text>
              </Column>
            </Row>
            ...
            {avisos.map((aviso) => (
              <Row key={aviso.url}>
                <Column style={{ ...cell, width: '60%' }}>
                  <Link href={aviso.url} style={tituloLink}>
                    {aviso.titulo}
                  </Link>
                </Column>
                <Column style={{ ...cell, width: '25%' }}>
                  <Text style={cellText}>{aviso.dotacao}</Text>
                </Column>
                <Column style={{ ...cell, width: '15%' }}>
                  <Text style={cellText}>{aviso.taxaFinanciamento}</Text>
                </Column>
              </Row>
            ))}
          </Section>
...
AvisosAbertos.PreviewProps = { avisos: avisosExemplo } as AvisosAbertosProps;
